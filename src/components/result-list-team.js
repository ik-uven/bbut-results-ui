...
class ResultListTeam extends Component {
    ...
    render() {
        ...
        const lapNumbers = () => {
            ...
            const content = [<td key="-4">Lagnamn</td>, <td key="-3">#</td>, <td key="-2">Namn</td>, <td key="-1">Status</td>,<td key="0">Varv</td>];
        ...
        const totalTeams = this.state.bbutTeamResults
            .map((team) => {
                return (
                    <tr key={team.name}>
                        <td>{nbsp(team.name)}</td><td style={cellStyleR}>{team.totalLaps}</td>
                    </tr>
                )
            });
